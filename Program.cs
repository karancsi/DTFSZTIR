...
namespace DTFSZTIR_bead
{
    class Program
    {
        ...
        public static void Print_Res_Interval(Resourse[] res, int numberRes)
        {
            ...
            Console.WriteLine("Erőforrások rendelkezésreállásának időintervalumai:");
            for (int i = 0; i < numberRes; i++)
            {
                Console.WriteLine("{0}. erőforras intervallumok száma: {1}", i, res[i].intervalN);
                for (int j = 0; j < res[i].intervalN; j++)
                {
                    Console.WriteLine("Kezdete:{0} , Vége: {1}", res[i].interval[j].ST, res[i].interval[j].ET);
                }
            }
        }
        ...
        //Keresés - Fm
        public static void Search(Job[] job, int numberJob, Resourse[] res, int numberRes, int[] s, int STEP, int LOOP, int cut_mode, double[] w, int K)
        {
            int[] actual_sol = new int[numberJob];
            int[] best_sol = new int[numberJob];
            int[] sol_0 = new int[numberJob]; //bázis
            int[] sol_ext = new int[numberJob]; //legjobb szomszéd ütemterv
            ...
            set_S_to_S2(sol_0, s, numberJob); //kezdeti bazis
            set_S_to_S2(best_sol, s, numberJob); // legjobb ismert
            Simulation(job, numberJob, res, numberRes, best_sol, 0, cut_mode);

            Result(job, numberJob, numberRes, best_sol, best_f);
            ...
            for (int i = 1; i <= STEP; i++)
            {
                ...
                {
                    //szomszéd generálása
                    Neighbour(sol_0, actual_sol, numberJob);
                    //szimu
                    Simulation(job, numberJob, res, numberRes, actual_sol, 0, cut_mode);
                    //kiertekeles
                    Result(job, numberJob, numberRes, actual_sol, actual_f);

                    if (j == 0)
                    {//best szomszéd
                        set_F_to_F2(f_ext, actual_f, K);
                        set_S_to_S2(sol_ext, actual_sol, numberJob);
                    }
                    else
                    {
                        if (F(f_ext, actual_f, w, K) < 0)
                        {
                            set_F_to_F2(f_ext, actual_f, K);
                            set_S_to_S2(sol_ext, actual_sol, numberJob);
                        }
                ...
                set_S_to_S2(sol_0, sol_ext, numberJob);

                if (F(f_ext, actual_f, w, K) < 0)
                {
                    set_F_to_F2(best_f, f_ext, K);
                    set_S_to_S2(best_sol, sol_ext, numberJob);
                }
            }
            set_S_to_S2(s, best_sol, numberJob);
        }

        private static void set_F_to_F2(double[] f1, double[] f2, int k)
        {
            for (int i = 0; i < k; i++)
            {
                f1[i] = f2[i];
        ...
        {//pelda szomszedsagi operatorra
            ...
            Random rand = new Random();
            //teljes masolat
            set_S_to_S2(actual_sol, sol_0, numberJob);
            //modositas
